 ...
 */
async function getDnssecChain(name, { resolverIP = '127.0.0.1', resolverPort = 5350 }) {
  // TODO: better fqdn test
  if (!/^[a-zA-Z0-9_\-.]+$/m.test(name)) {
    throw new Error('Invalid name.');
  }
  const kskPath = path.resolve(__dirname, '../etc/hsd-ksk');
  let output = await exec(`delv @${resolverIP} -p ${resolverPort} -a ${kskPath} _443._tcp.${name} TLSA +rtrace +mtrace +nosplit`);
