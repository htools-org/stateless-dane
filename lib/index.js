...
class StatelessDANECertificate {
  ...
  async create() {
    ...
    //generate if no keyfile json
    if (this.publicKeyJson) {
      let parsed = {"n":Buffer.from(this.publicKeyJson.n, 'hex'), "e":Buffer.from(this.publicKeyJson.e,'hex')}
      let publicKey = rsa.publicKeyImport(parsed)
