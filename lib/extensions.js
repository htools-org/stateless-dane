...
class UrkelProofExtension extends x509.Extension {
   ...
   */
  inject(root, proofs) {
    this.len = proofs.length;
    this.root = root;
    this.proofs = proofs.slice();

    const encodedProofs = this.proofs.map(proof => proof.encode(blake2b, 256));
    const value = Buffer.concat([Buffer.from([this.len]), this.root, ...encodedProofs]);
   ...
   */
  static create(root, proofs) {
    const ext = new this();
    ext.inject(root, proofs);
    return ext;
  }
  ...
  getJSON(parsed = true) {
    ...
    const proofs = this.proofs.map(proof => {
  ...
  readValue(br) {
    ...
    for (let i = 0; i < this.len; i++) {
      this.proofs.push(Proof.readBR(br, blake2b, 256));
    }
